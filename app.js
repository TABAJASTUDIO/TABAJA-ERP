...
function shell(title,body){return `<div class="top"><div class="brand">Tabaja ERP <small>Alpha 30</small></div><div class="topnav"><button id="companyTop" data-top-menu="company">K: Company</button><button id="dataTop" data-top-menu="data">Y: Data</button><button id="exchangeTop" data-top-menu="exchange">Z: Exchange</button><button id="gotoTop">G: Go To</button><button id="importTop" data-top-menu="import">O: Import</button><button id="exportTop" data-top-menu="export">E: Export</button><button id="shareTop" data-top-menu="share">M: Share</button><button id="printTop" data-top-menu="print">P: Print</button><button id="helpTop" data-top-menu="help">F1: Help</button></div></div><div class="strip">${esc(title)}</div>${body}<div class="status"><span>Q: Quit</span><span>A: Accept</span><span>Esc: Back</span><span>Enter: Select</span></div>`}
...
function companyForm(editId=null){const old=editId?DB.companies().find(x=>x.id===editId):null;S.screen='companyForm';app.innerHTML=shell(old?'Company Alteration':'Company Creation',`<main class="hub"><form class="card" id="cf"><div class="form-grid"><div class="field"><label>Company Name *</label><input name="name" required value="${esc(old?.name||'')}"></div><div class="field"><label>Mailing Name</label><input name="mailing" value="${esc(old?.mailing||'')}"></div><div class="field full"><label>Address</label><textarea name="address">${esc(old?.address||'')}</textarea></div><div class="field"><label>Country</label><input name="country" value="${esc(old?.country||'Sierra Leone')}"></div><div class="field"><label>Financial year beginning</label><input type="date" name="fy" value="${esc(old?.fy||'2026-04-01')}"></div><div class="field"><label>Books beginning</label><input type="date" name="books" value="${esc(old?.books||'2026-04-01')}"></div><div class="field"><label>Base Currency</label><input name="currency" value="${esc(old?.currency||'NLe')}"></div></div><div class="actions"><button type="button" class="btn" id="cancel">Cancel</button><button class="btn primary">${old?'Alter Company':'Create Company'}</button></div></form></main>`);const back=()=>old?gateway():hub();cancel.onclick=back;cf.onsubmit=e=>{e.preventDefault();let o=Object.fromEntries(new FormData(cf)),cs=DB.companies();if(old){let i=cs.findIndex(x=>x.id===editId);if(i<0){toast('Company not found');return}cs[i]={...cs[i],...o,id:cs[i].id};DB.saveCompanies(cs);toast('Company altered successfully');gateway()}else{let c={id:uid('c'),...o};cs.push(c);DB.saveCompanies(cs);DB.save(c.id,seed());openCompany(c.id)}};setupFormKeyboard(cf,back)}
function gateway(){S.screen='gateway';let c=company(),d=data();app.innerHTML=shell(c.name,`<main class="screen"><div class="gateway"><section class="company-info"><div class="meta"><div><small>CURRENT PERIOD</small><b>${esc(c.fy)} onward</b></div><div><small>CURRENT DATE</small><b>${new Date().toLocaleDateString('en-GB')}</b></div><div><small>NAME OF COMPANY</small><h2>${esc(c.name)}</h2></div><div><small>DATE OF LAST ENTRY</small><b>${d.lastEntry||'No entries yet'}</b></div></div></section><aside class="panel"><div class="panel-title">Gateway of Tabaja ERP</div><div class="section-title">MASTERS</div><button class="menu-btn" data-key="c" id="create">${hk('Create','C')}</button><button class="menu-btn" data-key="a" id="alter">${hk('Alter','A')}</button><button class="menu-btn" data-key="h" id="coa">C${hk('hart of Accounts','H')}</button><div class="section-title">TRANSACTIONS</div><button class="menu-btn" data-key="v" id="vouchers">${hk('Vouchers','V')}</button><button class="menu-btn" data-key="y" id="daybook">Da${hk('y Book','Y')}</button><div class="section-title">REPORTS</div><button class="menu-btn" data-key="b" id="bs">${hk('Balance Sheet','B')}</button><button class="menu-btn" data-key="p" id="pl">${hk('Profit & Loss A/c','P')}</button><button class="menu-btn" data-key="s" id="stock">${hk('Stock Summary','S')}</button><button class="menu-btn" data-key="d" id="display">${hk('Display More Reports','D')}</button><button class="menu-btn" data-key="o" id="dashboard">Dashb${hk('oard','O')}</button><div class="section-title"></div><button class="menu-btn" data-key="q" id="close">${hk('Quit','Q')}</button></aside></div></main>`);bindMenu('.panel .menu-btn');create.onclick=createMenu;alter.onclick=alterMenu;coa.onclick=chart;display.onclick=displayMenu;close.onclick=()=>quitPopup('Quit?',()=>{sessionStorage.clear();S.user=null;S.companyId=null;login()});vouchers.onclick=()=>{veResetDraft();voucherEntry()};['daybook','bs','pl','stock','dashboard'].forEach(id=>document.getElementById(id).onclick=()=>toast('This module is prepared for the next build.'));bindTopNav();gotoTop.onclick=goToPopup}

function bindTopNav(){
 document.querySelectorAll('[data-top-menu]').forEach(b=>b.onclick=()=>openTopMenu(b.dataset.topMenu,b));
 const g=document.getElementById('gotoTop');if(g)g.onclick=goToPopup;
}
function closeTopMenu(){document.getElementById('topMenuOv')?.remove()}
function topMenuItems(kind){
 const current=company();
 const maps={
  company:[
   {name:'Create',key:'C',run:()=>companyForm()},
   {name:'Alter',key:'A',run:()=>current?companyForm(current.id):companySelectForAlter()},
   {name:'Change',key:'F3',run:hub},
   {name:'Select',key:'Alt+F3',run:hub},
   {name:'Shut',key:'Ctrl+F3',run:()=>{sessionStorage.removeItem('te_company');S.companyId=null;hub()}},
   {heading:'USER MANAGEMENT'},{name:'Change User',disabled:true},{name:'Change Password',disabled:true},{name:'Users and Passwords',disabled:true},{name:'User Roles',disabled:true},{heading:'CONFIGURE'},{name:'Features',key:'F11',disabled:true},{name:'Security',disabled:true}
  ],
  data:[{heading:'COMPANY DATA'},{name:'Backup',run:backupAll},{name:'Restore',run:()=>restoreInput.click()},{name:'Split',disabled:true},{heading:'TROUBLESHOOTING'},{name:'Repair',disabled:true},{name:'Migrate',disabled:true},{heading:'CONFIGURE'},{name:'Data Path',disabled:true}],
  exchange:[{heading:'EXCHANGE'},{name:'Synchronise',disabled:true},{name:'Remote Access',disabled:true}],
  import:[{heading:'COMPANY DATA'},{name:'Masters',disabled:true},{name:'Transactions',disabled:true},{heading:'BANK DETAILS'},{name:'Bank Statement',disabled:true},{name:'Configuration',disabled:true}],
  ...
  help:[{name:'Tabaja Help',key:'Ctrl+F1',disabled:true},{name:"What's New",disabled:true},{name:'Upgrade',disabled:true},{heading:'SUPPORT'},{name:'Troubleshooting',disabled:true},{name:'Settings',disabled:true},{name:'About',run:()=>toast('Tabaja ERP Alpha 20')} ]
 };return maps[kind]||[]
}
...
function openTopMenu(kind,anchor){
 closeTopMenu();let items=topMenuItems(kind),x=document.createElement('div');x.id='topMenuOv';x.className='top-menu-overlay';
 x.innerHTML=`<div class="top-menu-dropdown">${items.map((it,i)=>it.heading?`<div class="top-menu-heading">${esc(it.heading)}</div>`:`<button class="top-menu-row ${it.disabled?'disabled':''}" data-i="${i}" ${it.disabled?'disabled':''}><span>${esc(it.name)}</span><kbd>${esc(it.key||'')}</kbd></button>`).join('')}</div>`;
 document.body.appendChild(x);let r=anchor.getBoundingClientRect(),menu=x.firstElementChild;menu.style.left=Math.max(4,Math.min(r.left,window.innerWidth-menu.offsetWidth-4))+'px';menu.style.top=r.bottom+'px';
 const rows=[...x.querySelectorAll('.top-menu-row:not([disabled])')];let idx=0;const set=n=>{if(!rows.length)return;idx=(n+rows.length)%rows.length;rows.forEach((b,j)=>b.classList.toggle('active',j===idx));rows[idx].focus()};
 rows.forEach((b,j)=>b.onclick=()=>{let it=items[Number(b.dataset.i)];closeTopMenu();it?.run?.()});
 x.onmousedown=e=>{if(e.target===x)closeTopMenu()};x.onkeydown=e=>{if(e.key==='Escape'){e.preventDefault();closeTopMenu();anchor.focus()}else if(e.key==='ArrowDown'){e.preventDefault();set(idx+1)}else if(e.key==='ArrowUp'){e.preventDefault();set(idx-1)}else if(e.key==='Enter'){e.preventDefault();rows[idx]?.click()}};setTimeout(()=>set(0),0)
}
...
const V={type:'Purchase',mode:'Item Invoice',optional:false,postDated:false,activeCell:0,focusId:null,editingItemIndex:null,reverseItemEdit:false,voucherDate:new Date().toISOString().slice(0,10),suppressContextOnce:false,editOriginal:'',editOriginalDate:'',items:[],values:{supplierNo:'',supplierDate:'',party:'',purchaseLedger:'',item:'',qty:'',rate:'',amount:'',narration:''},selected:{party:null,purchaseLedger:null,item:null}};
...
function veResetDraft(){
 ...
 V.voucherDate=new Date().toISOString().slice(0,10);V.editOriginal='';V.editOriginalDate='';V.items=[];V.editingItemIndex=null;V.reverseItemEdit=false;
...
function veRawCell(i){let c=VOUCHER_CELLS[i],v=V.values[c.key]||'',readonly=c.key==='amount';return `<div class="ve-cell ve-value${readonly?' calculated':''}" id="veCell${i}" data-i="${i}" data-kind="${c.kind}" tabindex="${i===V.activeCell?'0':'-1'}" contenteditable="${readonly?'false':'true'}" spellcheck="false">${esc(v)}</div>`}
function veCommittedRows(){return V.items.map((x,i)=>`<div class="voucher-item-row voucher-committed-row" data-row="${i}"><div class="voucher-item-name-cell"><div class="ve-static ve-entered-value">${esc(x.name)}</div><small>${x.kg?'('+Number(x.kg).toFixed(2)+' KG)':''}</small></div><div class="voucher-qty-cell"><div class="ve-static ve-entered-value right">${Number(x.qty).toFixed(2)} ${esc(x.unit||'CTN')}</div><small>${x.kg?'('+(Number(x.kg)*Number(x.qty)).toFixed(2)+' KG)':''}</small></div><div class="ve-static ve-entered-value right">${Number(x.rate).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</div><div class="voucher-unit ve-entered-value">${esc(x.unit||'CTN')}</div><div class="ve-static ve-entered-value right">${Number(x.amount).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</div></div>`).join('')}
...
function veItemKg(){let direct=Number(V.selected.item?.kg||V.selected.item?.weightKg||0);if(direct)return direct;let n=String(V.selected.item?.name||V.values.item||'').match(/(\d+(?:\.\d+)?)\s*\*?\s*KG/i);return n?Number(n[1]):0}
...
function veMoveBack(){
 let next=V.activeCell-1;
 // While revisiting an existing item, Amount is part of the reverse path.
 if(next===7&&V.editingItemIndex===null)next=6;
 if(next<0){veCloseContext(false);veResetDraft();gateway();return}
 veActivate(next)
}
...
function veRestoreLastCommittedItem(){
 ...
 // Restore the immediately preceding completed row as one protected cell-edit
 // state. Tally returns first to Amount, then Rate, Quantity, and finally Item.
 // It must never skip directly to the Item cell or Purchase Ledger.
 ...
 setTimeout(()=>{
  ...
  vePlaceCaretEnd(amount);
 },0);
 return true
}
function veClearCurrentItem(){V.values.item='';V.values.qty='';V.values.rate='';V.values.amount='';V.selected.item=null;V.editingItemIndex=null;V.reverseItemEdit=false}
...
function veHandleCellEscape(cellIndex){
 ...
 // Quantity follows Tally's edit rule: Backspace removes digits one by one,
 // while Escape clears the full figure and keeps focus in Quantity so a new
 // value can be typed from zero. A second Escape on an already-empty field
 // continues the normal reverse navigation.
 if(cellIndex===5){
  const quantity=document.getElementById('veCell5');
  const text=String(quantity?.textContent??V.values.qty??'').trim();
  if(text){
   V.values.qty='';
   V.values.amount='';
   if(quantity){quantity.textContent='';quantity.focus({preventScroll:true});vePlaceCaretEnd(quantity)}
   veRecalculate();
   V.editOriginal='';
   return true;
  }
...
function veCellKeydown(e){
 ...
 if(e.key==='Backspace'){
  ...
  // intentionally different from Party/Purchase Ledger selection fields.
  if(kind==='stock'&&cellIndex===4&&V.reverseItemEdit){
   if(e.currentTarget.textContent.length===0){
    e.preventDefault();e.stopPropagation();
    // The current restored item has now been erased. A further Backspace
    // must step straight into the previous committed row at Amount; it must
    // not force the user through End of List first.
    V.values.item='';V.values.qty='';V.values.rate='';V.values.amount='';
    V.selected.item=null;V.editingItemIndex=null;V.reverseItemEdit=false;
    if(V.items.length){veRestoreLastCommittedItem();return}
    veOpenContext('','stock');
   }
   return
  }
  if((kind==='ledger'||kind==='stock')&&String(e.currentTarget.textContent||'').trim()!==''){
   e.preventDefault();e.stopPropagation();
   V.values[cellDef.key]='';V.selected[cellDef.key]=null;e.currentTarget.textContent='';
   if(cellDef.key==='party')document.getElementById('partyBalance').textContent='';
...
function veCreateMaster(kind,prefill){
 ...
 let x=tallyPopup('quickCreate',title,kind==='stock'?`<form id="quickMasterForm" class="quick-master"><label>Name<input name="name" value="${esc(prefill)}" required></label><label>Unit<input name="unit" value="CTN" required></label><label>Weight per Unit (KG)<input name="kg" type="number" step="0.01" value="0"></label><label>Opening Quantity<input name="opening" type="number" step="0.01" value="0"></label><small>Ctrl+A: Accept &nbsp; Esc: Cancel</small></form>`:`<form id="quickMasterForm" class="quick-master"><label>Name<input name="name" value="${esc(prefill)}" required></label><label>Under<select name="groupId">${groupOptions}</select></label><label>Opening Balance<input name="opening" type="number" step="0.01" value="0"></label><label>Balance Type<select name="balanceType"><option>Dr</option><option>Cr</option></select></label><small>Ctrl+A: Accept &nbsp; Esc: Cancel</small></form>`);
 let f=x.querySelector('form');
 f.insertAdjacentHTML('beforeend',`<div class="quick-accept" hidden><div class="quick-accept-title">Accept?</div><div class="quick-accept-options"><button type="button" data-answer="yes" class="active">Yes</button><button type="button" data-answer="no">No</button></div></div>`);
 let confirmBox=f.querySelector('.quick-accept'),confirmButtons=[...f.querySelectorAll('.quick-accept-options button')],confirmIndex=0;
 const setConfirm=n=>{confirmIndex=(n+confirmButtons.length)%confirmButtons.length;confirmButtons.forEach((b,i)=>b.classList.toggle('active',i===confirmIndex));confirmButtons[confirmIndex].focus()};
 const showConfirm=()=>{if(!f.reportValidity())return;confirmBox.hidden=false;setConfirm(0)};
 const hideConfirm=()=>{confirmBox.hidden=true;let fields=[...f.elements].filter(el=>el.matches('input,select'));fields.at(-1)?.focus()};
 const accept=()=>{if(!f.reportValidity())return;let o=Object.fromEntries(new FormData(f));let made;if(kind==='stock'){made={id:uid('s'),name:o.name.trim(),unit:o.unit||'CTN',kg:Number(o.kg||0),opening:Number(o.opening||0),closing:Number(o.opening||0).toFixed(2)+' '+(o.unit||'CTN')};d.stockItems.push(made)}else{made={id:uid('l'),name:o.name.trim(),alias:'',groupId:o.groupId,group:d.groups.find(g=>g.id===o.groupId)?.name||'',opening:Number(o.opening||0),balanceType:o.balanceType||'Dr'};d.ledgers.push(made)}saveData(d);x.remove();V.activeCell=prevCell;let c=VOUCHER_CELLS[prevCell];V.values[c.key]=made.name;V.selected[c.key]=made;voucherEntry();V.suppressContextOnce=true;setTimeout(()=>{veCloseContext();veMove(1)},0)};
 confirmButtons[0].onclick=accept;confirmButtons[1].onclick=hideConfirm;
 f.onsubmit=e=>{e.preventDefault();showConfirm()};
 x.onkeydown=e=>{
  if(!confirmBox.hidden){
   if(e.key==='Escape'){e.preventDefault();hideConfirm()}
   else if(e.key==='ArrowLeft'||e.key==='ArrowUp'){e.preventDefault();setConfirm(confirmIndex-1)}
   else if(e.key==='ArrowRight'||e.key==='ArrowDown'){e.preventDefault();setConfirm(confirmIndex+1)}
   else if(e.key==='Enter'){e.preventDefault();confirmIndex===0?accept():hideConfirm()}
   return
  }
  if(e.key==='Escape'){e.preventDefault();x.closePopup()}
  else if(e.ctrlKey&&e.key.toLowerCase()==='a'){e.preventDefault();accept()}
  else if(e.key==='Enter'){
   let fields=[...f.elements].filter(el=>el.matches('input,select')),i=fields.indexOf(document.activeElement);
   if(i>=0){e.preventDefault();if(i<fields.length-1){fields[i+1].focus();fields[i+1].select?.()}else showConfirm()}
  }
 };
 setTimeout(()=>f.elements.name.select(),0)
}
...
function backupAll(){let out={version:'alpha-13',companies:DB.companies(),data:{}};out.companies.forEach(c=>out.data[c.id]=DB.data(c.id));let a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(out,null,2)],{type:'application/json'}));a.download='tabaja-erp-alpha-13-backup.json';a.click()}
...
const KeyboardEngine={
 enabled:true,
 isTypingTarget(el=document.activeElement){return !!el&&(['INPUT','TEXTAREA','SELECT'].includes(el.tagName)||el.isContentEditable||el.getAttribute?.('contenteditable')==='true')},
 ...
 handleVoucher(e){
  ...
  const k=this.key(e);
  const activeVoucherCell=e.target?.closest?.('.ve-cell');
  const panel=document.getElementById('voucherContextPanel');
  const listOpen=panel&&!panel.hidden&&activeVoucherCell;
  if(listOpen&&['ARROWDOWN','ARROWUP','ENTER'].includes(k)){
   this.stop(e);
   if(k==='ARROWDOWN')veListMove(1);
   else if(k==='ARROWUP')veListMove(-1);
   else veListPick();
   return true;
  }
  if(k==='ESCAPE'){
   this.stop(e);
   const cell=activeVoucherCell;
   // During voucher entry, a render can momentarily move DOM focus away from
   // the cell. Use V.activeCell instead of treating that as an exit request.
   veHandleCellEscape(cell?Number(cell.dataset.i):V.activeCell);
   return true
  }
 ...
 handleGlobal(e){
  ...
  if(!typing&&['K','Y','Z','O','E','M','P'].includes(k)){let map={K:'company',Y:'data',Z:'exchange',O:'import',E:'export',M:'share',P:'print'};let b=document.querySelector(`[data-top-menu="${map[k]}"]`);if(b){this.stop(e);openTopMenu(map[k],b);return true}}if(k==='F1'&&!typing){let b=document.querySelector('[data-top-menu="help"]');if(b){this.stop(e);openTopMenu('help',b);return true}}if(k==='F4'||(k==='G'&&!typing)){this.stop(e);goToPopup();return true}
  const ov=document.getElementById('ov');
 ...
 dispatch(e){
  ...
  if(document.getElementById('gotoOv')||document.getElementById('tallyOv')||document.getElementById('topMenuOv'))return;
